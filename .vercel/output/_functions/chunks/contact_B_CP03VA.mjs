...
const POST = async ({ request }) => {
  const apiKey = "re_FSn1VTWK_HQHGYuX5TWB4omLZvZLkdZn4";
  ...
  try {
    ...
    const serviceLabel = service ? service.charAt(0).toUpperCase() + service.slice(1) : "Non spécifié";
    const subject = lang === "es" ? `Nueva solicitud de contacto - ${name}` : `Nouvelle demande de contact - ${name}`;
    const html = `
		<!doctype html>
		<html lang="${lang}">
			...
			<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
				...
				<div style="text-align: center; margin-top: 20px; font-size: 12px; color: #9ca3af;">
					© D.E.B TOUT CORPS — ${(/* @__PURE__ */ new Date()).getFullYear()}
				</div>
			</body>
		</html>
		`;
